...
export const dataProvider = ({
  ...
  return {
    ...
    getList: async (resource, params) => {
      let c = collection(firestore, resource);
      return getDocs(query(c)).then(getResults).then(packageResults).catch(mapError);
    },
    ...
    getMany: (resource, params) => {
      ...
      return getDocs(query(collection(firestore, resource), where(targetField, "in", ids))).then(getResults).then(results => results.map(r => ({
        ...r,
        id: r[targetField]
      }))).then(packageResults).catch(mapError);
    },
