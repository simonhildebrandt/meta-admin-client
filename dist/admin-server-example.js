...
app.post("/auth", async (req, res) => {
  ...
  if (token == masterKey) {
    admin.auth().createCustomToken('admin').then(customToken => {
      res.status(200).json({
        customToken
      });
    }).catch(error => {
...
async function verifyToken(req, res, next) {
  ...
  if (!idToken) return res.status(403).json({
    message: 'no token'
  });
  const a = admin.auth();
  a.verifyIdToken(idToken).then(_ => next()) // Passing anything to `next` is an error
  .catch(error => {
    res.status(403).json({
